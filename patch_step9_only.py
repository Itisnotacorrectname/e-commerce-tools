...
idx9 = data.find(old9)
if idx9 < 0:
    print('step9 NOT FOUND')
    target = b'var missingEgeo = s12 ? (s12.egeoFeatures || []).filter(function(f) { return f.missing; }) : [];'
    tidx = data.find(target)
    print('  missingEgeo at byte', tidx)
    if tidx >= 0:
        print('  context:', repr(data[tidx:tidx+50]))
    sys.exit(1)

print('step9 found at byte', idx9)
data = data[:idx9] + new9 + data[idx9+len(old9):]
with open(path, 'wb') as f:
    f.write(data)
print('step9 patched OK, new size:', len(data))
