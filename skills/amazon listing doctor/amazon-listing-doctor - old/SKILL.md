...
version: 2.4
description: Diagnose any Amazon listing with a full competitive audit (Route B), Rufus intent simulation, Cosmo content scoring, rewritten bullet copy, and Kane private KB integration. Paste any amazon.com product link to run.
note: v2.3 data layer (Step 1-4 crawl) + KB-enhanced analysis (Step 5-14 via SKILL.md + private KB). Fusion of e-commerce-tools v2.3 + skills v5.2 KB.
---
...
## Private Knowledge Base（Kane 私人知识库）

> ⚠️ 此目录为 Kane 私人研究资料，不随 skill 分发。

**路径：** `C:/Users/csbd/.openclaw/workspace/亚马逊分析知识库/knowledge/`

| 文件 | 内容 | 关键用途 |
|------|------|---------|
| `rufus_links/` | 11篇 Rufus/Cosmo/PPC 文章 | Step3/6/10 背景 |
| `2511.20867v1.kb.txt` | E-GEO benchmark 论文蒸馏 | Step13 V16 |
| `p15.kb.txt` | Q&A推荐系统规则 | Step13 V9/V10 |
| `listing显性违规识别知识库.txt` | V1-V8 完整规则 | Step12 |
| `listing隐性违规识别知识库.txt` | V9-V16 完整规则 | Step13 |

**调用格式：** 在诊断报告中引用为 `参考: knowledge/research/2511.20867v1.kb.txt §3`

**动态调取规则：** 详见 `references/kb_retrieval_rules.md`。每个 Step 按需从 Kane 私人知识库调取对应文件，不要一次性全量加载。
...
```
<tool>exec</tool>
command: ls {CHECKPOINT_DIR}/{ASIN}/
```
