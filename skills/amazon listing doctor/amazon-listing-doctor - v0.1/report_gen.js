...
function buildHtml(asin) {
  ...
  // ── Step10 ──
  const rufusQuestions = s10?.questions || [];
  const cosmoScores    = s10?.scores || [];
  const rufusAvgScore  = s10?.averageScore ? s10.averageScore.toFixed(1) : 'N/A';
...
<!-- SECTION 3: Keyword Universe (self-only mode — no competitor scraping) -->
<h2>3. Keyword Universe</h2>
<div class="card">
<p style="color:var(--muted);font-size:.85rem;margin:0 0 12px 0;">Self-only analysis — keyword data extracted from this listing only. Competitor benchmark skipped.</p>
...
<h2>8. Rufus User Intent + Cosmo Score <span style="font-size:.75rem;color:var(--accent2);"> | ${(s10?.kbMethod || '').indexOf('bullet') === 0 ? 'Bullet Analyzer' : 'KB Rules (Fallback)'}</span></h2>
<div class="card">
...
<!-- SECTION 9:显性违规 -->
<h2>9. Explicit Violations (${violations.length})</h2>
<div class="card">
...
<div class="violation-match">${v.bullet ? 'Bullet ' + v.bullet + ' — ' : ''}"${v.matched || ''}"</div>
</div>
