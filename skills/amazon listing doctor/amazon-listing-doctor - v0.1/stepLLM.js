  ...
  return new Promise(function(resolve, reject) {
    ...
    var req = http.request({
      ...
      res.on('end', function() {
        ...
        } catch(e) { console.log('[llmRequest] parse err:', e.message, 'data:', data.substring(0,100)); resolve(null); }
      });
    });
    req.on('error', function(e) { console.log('[llmRequest] network err:', e.message); reject(e); });
...
async function analyzeListing(listing) {
  ...
  var llmResult = await callLLMWithRetry(userPrompt, systemPrompt, 1, 55000);
...
module.exports = { analyzeListing, callLLMWithRetry, buildSystemPrompt };
