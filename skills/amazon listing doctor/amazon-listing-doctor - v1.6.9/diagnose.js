...
async function main() {
  ...
  // ── v1.6.9: 数据完整性验证 ────────────────────────────────
  if (!shouldSkip(4)) {
    var priceCap = s4.priceCaptureRate || 0;
    var priceFound = s4.priceFound || 0;
    var totalComp = s4.totalFound || 0;
    log('  📊 Price capture: ' + priceFound + '/' + totalComp + ' = ' + (priceCap * 100).toFixed(0) + '%');
    if (priceCap < 0.3 && totalComp >= 5) {
      log('  🔴 ERROR: Price capture rate < 30% — possible geo-redirect. Aborting.');
      log('  → 解决：更换美国 IP 后重新执行 node diagnose.js ' + asin + ' --force');
      process.exit(1);
    }
  }
