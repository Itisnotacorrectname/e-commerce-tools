  ...
  try {
    ...
    for (var attempt = 0; attempt < 6; attempt++) {
      widgetFound = await page.evaluate(function() {
        return !!document.querySelector('#contextualIngressPtLabel_deliveryShortLine, #contextualIngressPt, #nav-global-location-popover-link');
      });
    ...
    // Use evaluate to click — more reliable than page.$() when widget is inside a popover trigger
    var clicked = await page.evaluate(function() {
      var btn = document.querySelector('#contextualIngressPtLabel_deliveryShortLine, #contextualIngressPt, #nav-global-location-popover-link');
    ...
    // Click the confirmation "Done" button to finalize the ZIP change
    var doneBtn = await page.$('button.a-button-text');
    if (doneBtn) {
      var doneText = await doneBtn.textContent();
      if (doneText && doneText.trim() === 'Done') {
        await doneBtn.click();
  ...
  if (zipWorked) {
    ...
    for (var v = 0; v < 8; v++) {
      var dtText = await page.evaluate(function() {
        var w = document.querySelector('#contextualIngressPtLabel_deliveryShortLine, #contextualIngressPtLabel, #contextualIngressPt, #nav-global-location-popover-link');
    ...
    if (!zipVerified) {
      ...
      // ZIP verified — reinforce cookies and continue (no reload to avoid triggering geo-redirect)
      await cookieInject(page, config);
      await page.waitForTimeout(1000);
      return 'zip';
    }
...
async function scrapeProductPage(asin, cc) {
  ...
  try {
    ...
    try {
      ...
      // Timeout or redirect — retry once after cookie inject
      await cookieInject(page, config);
      await page.waitForTimeout(2000);
    ...
    if (cc === 'US') {
      await zipAndReload(page, config, 30000);
    }
...
async function scrapeCompetitorSearch(keyword, cc, opts) {
  ...
  try {
    browser = await createBrowser(cc);
    // Create ONE page and apply ZIP ONCE — reuse for all cascade rounds
    let currentPage = await browser.newPage();

    // For US: set cookies before any navigation
    if (cc === 'US') {
      await cookieInject(currentPage, config);
    ...
    for (let ri = 0; ri < plan.length; ri++) {
      ...
      try {
        ...
        // Navigate (may timeout on geo-redirect — that's OK)
        try {
          await currentPage.goto(searchUrl, { waitUntil: 'domcontentloaded', timeout: searchTimeout });
        } catch(e) {
          await cookieInject(currentPage, config);
          await currentPage.waitForTimeout(2000);
          await currentPage.goto(searchUrl, { waitUntil: 'domcontentloaded', timeout: searchTimeout }).catch(function() {});
        }

        // Detect geo-redirect — if URL is no longer amazon.com, create fresh page
        if (cc === 'US') {
          const urlAfterNav = currentPage.url();
          if (urlAfterNav.indexOf('amazon.com') === -1) {
            await currentPage.close().catch(function() {});
        ...
        try {
          const spCcAccept = await currentPage.$('#sp-cc-accept');
          if (spCcAccept) {
            try { await spCcAccept.click({ timeout: 3000 }); await currentPage.waitForTimeout(2000); } catch(e) {}
          }
        } catch(e) {}

        await handleInterstitial(currentPage);

        // Extract via page.evaluate
        const comps = await currentPage.evaluate(function(opts) {
          ...
          results.forEach(function(el, i) {
            ...
            var isAd = el.getAttribute('data-ad') || el.getAttribute('data-sponsored') || el.querySelector('[data-ad], [data-sponsored]');
            if (isAd) return;
            ...
            if (priceEl) {
              var p = priceEl.textContent.trim();
              if (p && /^[\$\u20AC\£\¥]/.test(p) && /[0-9]/.test(p)) { price = p; rawPriceText = p; }
            }
            if (!price) {
              var allOff = el.querySelectorAll('.a-offscreen');
              for (var oi = 0; oi < allOff.length; oi++) {
                var txt = allOff[oi].textContent.trim();
                if (txt && /^[\$\u20AC\£\¥]/.test(txt) && /[0-9]/.test(txt)) { price = txt; rawPriceText = txt; break; }
              }
            }
            if (!price) {
              var tc = el.textContent || '';
              var m = tc.match(/(?:^|[^\w])(?:[\$\u20AC\£\¥]|CNY|SGD|EUR|GBP)[\s]*[0-9][0-9,.]{2,}/);
              if (m) { price = m[0].trim(); rawPriceText = m[0].trim(); }
            }
            // For US marketplace: reject prices with any CNY/SGD indicator anywhere in text
            // (handles ¥CNY prefix, CNY code prefix, and other currency patterns)
            if (price && opts.cc === 'US') {
              if (/CNY|SGD|^\u00A5/.test(rawPriceText)) { price = ''; }
            }
