...
1. **版本化修改** — 所有修改必须在新的版本号目录下进行，不允许在原版本目录直接修改。每次功能/修复修改，创建新的 `amazon-listing-doctor - v{N}.X` 目录，基于上一个版本修改。
2. **每次修改前备份** — `diagnose.js → diagnose.js.bak`，每次改动前执行
3. **sandbox 测试** — 修改先在 `sandbox/` 目录验证，不直接在 skill 目录测试
4. **通用逻辑** — 所有逻辑针对任意产品，不得硬编码特定产品词/品类词
5. **爬取逻辑独立** — `step2_worker.js` / `step4_worker.js` 等爬取模块与报告生成解耦，修改爬取不影响报告渲染

### 版本号规则
- **大版本（如 v1.8 → v1.9）**：重大架构调整、新增功能、API 变更
- **小版本（如 v1.8.1 → v1.8.2）**：Bug 修复、逻辑优化、细节调整
- **每次提交前** 确认改动内容属于哪个版本级别
- **版本目录结构**：`amazon-listing-doctor - v{N}.X`，与 Git tag 对应
