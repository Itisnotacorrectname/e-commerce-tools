#!/usr/bin/env node
// ─────────────────────────────────────────────────────────────
//  Amazon Listing Doctor — diagnose.js v1.8 (Route B)
//
...
async function step4(s1, s2, s3) {
  ...
  // ── spawn 子进程的通用封装 ────────────────────────────────
  // 注：categoryFallback 固定传空字符串用于重试，避免 undefined 引用
...
async function main() {
  ...
  console.log('');
  var skipAgent = process.argv.includes('--no-agent');
  if (!skipAgent) {
    // ── Phase 2: 自动触发 Claude Agent 执行分析 ────────────
    console.log('');
    console.log('▶ Phase 2: Spawning Claude Agent for analysis...');
    console.log('▶ Phase 2: Analysis will be executed in current session');
    console.log('   (No sub-agent needed — AI executes Phase 2 directly after data layer)');
  } else {
