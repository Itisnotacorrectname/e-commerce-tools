  ...
  for (var i = 0; i < words.length - 1; i++) {
    if (!STOPWORDS.has(words[i]) && !STOPWORDS.has(words[i + 1]) &&
      ...
      if (PRODUCT_TYPE_WORDS.has(words[i]) || PRODUCT_TYPE_WORDS.has(words[i + 1])) {
        var w1IsProduct = PRODUCT_TYPE_WORDS.has(words[i]);
        var w2IsProduct = PRODUCT_TYPE_WORDS.has(words[i + 1]);
        // 评分：两个词都是产品类型 = 2分（最高），1个是 = 1分
        // 这样 "sectional sofa" 会排在 "shaped sectional" 前面
        var score = (w1IsProduct ? 1 : 0) + (w2IsProduct ? 1 : 0);
        bigrams.push({ phrase: words[i] + ' ' + words[i + 1], score: score });
      }
    }
  }
