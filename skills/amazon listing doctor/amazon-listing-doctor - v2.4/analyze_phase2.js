...
async function main() {
  ...
  var keywords = analyzeKeywords(s2, s4);

  log('Step 6: Title audit...');
  var titleAudit = auditTitle(s2);

  log('Step 7: Title optimization (LLM)...');
  var optimizedTitles = await generateTitles(s2, keywords);

  log('Step 8: Backend keywords...');
  var backend = generateBackend(s2, keywords);
  await sleep(2000);

  log('Step 9: Bullet rewriting (LLM)...');
  var bullets = await generateBullets(s2);

  log('Step 10: Rufus questions (LLM)...');
  var questions = await generateRufusQuestions(s2, keywords);
  await sleep(2000);

  log('Step 11: Cosmo scoring (LLM)...');
  var cosmo = await generateCosmo(s2, questions);

  log('Step 12: Violation detection...');
  var violations = detectViolations(s2);

  log('Step 13: Listing weight...');
  var listingWeight = analyzeListingWeight(s2);

  log('Step 14: Action plan...');
  var actionPlan = generateActionPlan(s4, titleAudit, cosmo, violations, listingWeight);
