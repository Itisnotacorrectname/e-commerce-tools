...
async function main() {
  ...
  if (reviewCount < 10) {
    ...
    saveCp(asin, 13, s13);
  } else if (shouldSkip(13)) {
    s13 = loadCp(asin, 13);
    log('▶ Step 13: Review Analysis (cached — ' + s13.totalExtracted + ' reviews)');
  } else {
  ...
  saveCp(asin, 15, {
    asin: asin,
    reviews: s13.reviews || [],
    totalExtracted: s13.totalExtracted || 0,
    ratingValue: s13.ratingValue || null,
    reviewCount: s13.reviewCount || String(reviewCount),
    starDistribution: s13.starDistribution || {},
    scrapeError: s13.scrapeError || null,
    skipped: s13.skipped || null,
    scrapedAt: new Date().toISOString()
  });
  ...
  // ── v2.4: 自动 Phase 2 检测 ───────────────────────────────
  // 当 AI 调用 diagnose.js 时（OPENCLAW_AGENT=1），自动执行 Phase 2
  if (process.env.OPENCLAW_AGENT === '1') {
    console.log('  [auto] OPENCLAW_AGENT=1 — 自动执行 Phase 2');
    ...
    if (fs.existsSync(analyzePath)) {
      ...
      console.error('  [auto] analyze_phase2.js not found — skipping Phase 2');
      process.exit(0);
    }
    return;
  }
...
(function() {
  if (process.env.OPENCLAW_AGENT === '1') {
    var agentAsin = process.argv[2];
    if (agentAsin) {
      console.log('[diagnose] Phase 1 complete, spawning analyze_phase2.js for ' + agentAsin + '...');
      var child = require('child_process').spawn(process.execPath, ['analyze_phase2.js', agentAsin], {
        cwd: __dirname, stdio: 'inherit', env: Object.assign({}, process.env, {OPENCLAW_AGENT: '0'})
      });
      child.on('exit', function(code) {
        if (code === 0) console.log('[diagnose] Phase 2 complete.');
        else { console.error('[diagnose] Phase 2 failed with code ' + code); process.exit(code); }
      });
      process.exit(0);
    }
  }
