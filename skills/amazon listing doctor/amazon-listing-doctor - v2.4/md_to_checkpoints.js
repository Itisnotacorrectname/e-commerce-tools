...
function parseStep10(text) {
  ...
  if (json && json.questions) {
    // 规范化在 normalizeStep() 统一处理，这里只返回原始解析结果
    return json;
  }

  var questions = [];
  text.split('\n').forEach(function(l) {
    var cleaned = l.replace(/^\s*(?:[Q\d]+[\.:]\s*|[-*]\s*)/, '').trim();
    if (cleaned.length < 15) return;
    if (!/\?|buyerType|decisionBarrier|代表买家|决策障碍/.test(cleaned)) return;
...
// ── 字段规范化：analysis.md 输出的各种字段名变体 → report_gen.js 期望的字段名 ──
function normalizeStep(stepKey, data) {
  if (!data) return data;
  var d = Array.isArray(data) ? data.slice() : Object.assign({}, data);

  switch (stepKey) {
    case 'step5':
      // competitorCount: 自动从 step4.json 读取（不依赖手动填的旧数据）
      try {
        var s4 = JSON.parse(fs.readFileSync(path.join(CHECKPOINT_DIR, process.argv[2], 'step4.json'), 'utf8'));
        d.competitorCount = (s4.totalFound || d.competitorCount) && (s4.totalFound || d.competitorCount);
      } catch(e) {}
      break;
    ...
    case 'step10':
      // questions 支持 { question, buyerType, decisionBarrier } 对象格式
      // 展平为字符串供 report_gen.js 的 deriveRufusContext 使用
      if (Array.isArray(d.questions)) {
        d.questions = d.questions.map(function(q) {
          if (typeof q === 'string') return q;
          var parts = [q.question || ''];
          if (q.buyerType) parts.push('[buyerType:' + q.buyerType + ']');
          if (q.decisionBarrier) parts.push('[decisionBarrier:' + q.decisionBarrier + ']');
          return parts.join(' ');
        });
      }
      break;
    ...
    case 'step12':
      ...
      if (Array.isArray(d.implicit)) {
        d.implicit = d.implicit.map(function(v) {
          var item = Object.assign({}, v);
          if (!item.explanation && item.description) item.explanation = item.description;
          if (!item.description && item.explanation) item.description = item.explanation;
          return item;
        });
      }
      break;
    ...
    case 'step13':
      // keyword/note → factor/current/action
      if (Array.isArray(d.issues)) {
        d.issues = d.issues.map(function(w) {
          var item = Object.assign({}, w);
          if (!item.factor && item.keyword) {
            item.factor = item.keyword;
    ...
    case 'step14':
      ...
      if (!d.pendingData) d.pendingData = [];
      break;
  }
  return d;
}
...
async function main() {
  ...
  for (var n = 5; n <= 14; n++) {
    ...
    try {
      var result = normalizeStep('step' + n, parser(sectionText));
