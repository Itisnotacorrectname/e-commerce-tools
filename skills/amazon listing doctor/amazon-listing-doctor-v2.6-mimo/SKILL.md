...
description: |
  当用户粘贴亚马逊链接或说「诊断」「诊断这个listing」「分析链接」时，自动调用本skill。
  功能：实时抓取亚马逊链接数据、抓取搜索页竞品数据、分析该listing的情况并给出评分和建议。
note: v2.6-mimo 基于 v2.3 清洁架构，加入 v2.4 的 category-weighted 核心产品提取优化，开源零依赖，不修改任何源文件。
---
...
**⚠️ 执行位置约束：Phase 2 分析必须在主 session 中执行，禁止使用子任务（subagent）。**
- Phase 2 需要完整上下文（SKILL.md 规则 + step2-4 数据 + 分析逻辑），子任务上下文割裂会导致分析不完整
- Phase 1（爬虫）可以在主 session 用 exec 直接运行 diagnose.js
- Phase 3（渲染）在主 session 用 exec 运行 report_gen.js
- 只有纯爬虫类独立任务（如批量诊断多个 ASIN）才考虑子任务
...
**若竞品抓取失败（step4.json 竞品数量 = 0）：** 立即停止 Phase 2 分析，输出错误提示：“竞品数据未能获取，无法继续诊断。请等待 10-15 分钟后重试（Amazon 反爬冷却），或更换 ASIN。”不生成任何 step5-14 文件，不生成报告。

**开源原则：** 本 skill 自包含所有分析规则（V1-V18 完整规则内联在 SKILL.md），不依赖外部 KB 文件。用户可选添加 knowledge/ 目录作为补充，但 skill 本身无需外部文件即可运行。

---

## 执行流程

### 路径常量（每次执行前用 exec 工具读取，不要硬编码）

```
<tool>exec</tool>
command: node -e "const os=require('os'),path=require('path');const ws=process.env.OPENCLAW_WORKSPACE||path.join(os.homedir(),'.openclaw','workspace');console.log('WORKSPACE='+ws);console.log('CHECKPOINT_DIR='+path.join(ws,'amazon-listing-doctor','checkpoints'));console.log('SKILL_DIR='+path.join(ws,'skills','amazon-listing-doctor'));"
```

将输出的三个路径值替换到后续所有 `{CHECKPOINT_DIR}` 和 `{SKILL_DIR}` 占位符中。

---

### Phase 1：调用数据层脚本

**⚠️ Phase 1 必须在主 session 中用 exec 执行，不要用子任务。**

```bash
node "{SKILL_DIR}\diagnose.js" [ASIN 或 Amazon URL]
```

**⚠️ 不要用 `cd <dir>; node` 模式**：OpenClaw exec preflight 会拒绝复杂解释器调用，必须用完整绝对路径直接调 node。
...
### ⚠️ Checkpoint 写入规范（每步必须执行，不可跳过）

**每完成一个分析步骤，必须立即调用 `write` 工具将结果写入对应文件。这是一个工具调用操作，不是文字描述。**

**路径规则：**
```
CHECKPOINT_DIR = {OPENCLAW_WORKSPACE}/amazon-listing-doctor/checkpoints/{ASIN}/
```
其中 `{OPENCLAW_WORKSPACE}` 从环境变量读取，默认为 `C:\Users\csbd\.openclaw\workspace`（Windows）。
...
| B. 标题审计 | step6.json |
| C. 三版优化标题 | step7.json |
| D. Backend Keywords | step8.json |
| G / G.5. Bullet 改写 + Fact-Check | step9.json |
| E. Rufus 意图模拟 | step10.json |
| F. Cosmo 内容评分 | step11.json |
| H + I. 显性 + 隐性违规 | step12.json |
| J. Listing Weight | step13.json |
| K. 优先行动计划 | step14.json |

**每步末尾必须按以下格式调用 `write` 工具：**

```
<tool>write</tool>
path: {CHECKPOINT_DIR}/{ASIN}/step{N}.json
content: {本步骤输出的完整 JSON 对象，严格符合下方 schema}
```

**写入约束：**
- JSON 必须严格符合本文件"Checkpoint JSON Schema"章节中对应步骤的字段定义
- 写入成功后在对话中输出一行确认：`✅ step{N}.json 已写入`
- 如果 write 工具调用失败，立即报错，不继续下一步
...
**方法：**
1. 对所有竞品标题做词频统计，去除以下词后再统计：
   - **品牌词**：识别方法——标题最左侧第一个词组（通常大写或全大写，如 ZINUS、Yaheetech、VECELO），以及 step2.json 的 `brand` 字段值。如不确定是否为品牌词，检查该词是否在 5 个以上竞品标题的相同位置（最左侧）出现——是则为品牌词，过滤掉
   - **Stopwords**：and, with, for, the, a, an, of, in, on, to, by, set, new, home
   - **单字母词**
2. 按出现频率分三级：
...
**若竞品数量 < 5：** 立即停止 Phase 2 分析，输出错误提示：“竞品数据不足（仅获取 N 条，需要 ≥5 条），无法继续诊断。请等待 10-15 分钟后重试，或更换 ASIN。”不生成任何 step5-14 文件，不生成报告。

**【write 工具调用】** 分析完成后立即执行：
```
<tool>write</tool>
path: {CHECKPOINT_DIR}/{ASIN}/step5.json
content: {"primary":[...],"secondary":[...],"backend":[...],"sizeSignals":[...],"competitorCount":N}
```
...
```
<tool>write</tool>
path: {CHECKPOINT_DIR}/{ASIN}/step6.json
content: {"issues":[...],"spellErrors":[...],"charCount":N,"charLimit":200,"brandAtStart":true}
```
...
```
<tool>write</tool>
path: {CHECKPOINT_DIR}/{ASIN}/step7.json
content: {"versionA":"...","versionAChars":N,"versionANote":"...","versionB":"...","versionBChars":N,"versionBNote":"...","versionC":"...","versionCChars":N,"versionCNote":"..."}
```
...
```
<tool>write</tool>
path: {CHECKPOINT_DIR}/{ASIN}/step8.json
content: {"backend":"word1 word2 word3 ...","byteCount":N,"charLimit":250}
```

---

#### G.5 Fact-Check（产品事实验证）

**执行时机：** 在 G（Bullet 完整改写）之后、写入 step9.json 之前执行。每条改写都必须通过此检查。

**规则：改写内容只能包含 step2.json 可验证的事实。**

| 改写内容类型 | 验证方式 | 无法验证时的处理 |
|------------|---------|----------------|
| 材质声明（如 "Chenille fabric"、"solid oak"） | 必须出现在 step2.json 的 `title` 或 `bullets` 字段中 | 降级为模糊表述（如 "fabric upholstery"）或标注 `[待确认]` |
| 尺寸/规格（如 "fits through 30\" doorways"、"250 lbs capacity"） | 必须出现在 step2.json 的 `title`、`bullets` 或 `sizeSignals` 中 | 删除或改写为"可配置尺寸（以实际产品参数为准）" |
| 认证声明（如 "CertiPUR-US certified"、"CARB compliant"） | 必须出现在原始 bullets 中 | 完全删除，不得推断或假设 |
| 功能声明（如 "fully reversible"、"modular"） | 必须出现在原始 bullets 或 title 中 | 删除或标注 `[待确认]` |

**执行流程：**

```
for each bullet rewrite:
  1. 提取改写中所有具体事实声明（材质、尺寸、认证、功能）
  2. 在 step2.json 的 title + bullets 中搜索对应依据
  3. 有依据 → 保留，在 explain 字段注明"来源：step2 title/bullet N"
  4. 无依据 → 降级改写 或 标注 [待确认] 并在 explain 字段说明原因
```

**Fact-Check 结果写入 step9.json：**

在每个 bullet 对象中加入 `factCheck` 字段：
...
```json
{
  ...
  "factCheck": {
    "passed": true,
    "claims": [
      { "claim": "Chenille fabric", "verified": true,  "source": "bullet 2" },
      { "claim": "fits through 30\" doorways", "verified": false, "action": "removed — no dimension data in step2" }
    ]
  }
}
```
...
原 bullet 改写包含：`"Chenille upholstery — soft-touch fabric that resists pilling, fits through standard 30" doorways for hassle-free delivery"`

Fact-Check：
- "Chenille" → step2 bullet 2 有 "chenille fabric" → ✅ 保留
- "fits through 30\" doorways" → step2 title/bullets 无任何门宽或模块宽度数据 → ❌ 删除，改写为"modular pieces designed for easy room delivery（实际尺寸请见规格页）"

> 📌 **核心原则：改写是优化表达，不是创造事实。**

**无法验证时的标准写法（⚠️ 重要）：**

绝对不要在改写正文里写 `[X] lbs`、`[待确认]`、`[N]` 这类占位符——运营会误认为系统错误。

正确做法是用括号内的中文行动提示替代：
- 承重数据缺失 → `（承重数据：请向供应商确认后填入，建议格式：supports up to ___ lbs）`
- 尺寸数据缺失 → `（具体尺寸：请核实产品规格页后填入）`
- 认证缺失 → 直接删除，不留占位符

同时在 `explain` 字段写：`⚠ 需卖家提供：[具体缺失数据类型]，待确认后替换括号内内容`

**【write 工具调用】** G + G.5 均完成后立即执行：
```
<tool>write</tool>
path: {CHECKPOINT_DIR}/{ASIN}/step9.json
content: {"bullets":[{"original":"...","rewrite":"...","explain":"...","factCheck":{"passed":true,"claims":[{"claim":"...","verified":true,"source":"..."}]}},...]}
```
...
- Frame questions from the BUYER'S situation as subject — NOT from the product's specs as subject
  ✅ "Do you host large outdoor dinners — how important is it that chairs stay stable on uneven ground?"
  ❌ "What is the exact weight capacity of the chairs?" (spec audit question, not Rufus)
- Do NOT ask about: price, shipping time, shipping method, packaging dimensions, box size,
  delivery speed, warranty, or return policy — these are logistics questions answered by
  Amazon's fulfillment system, not by listing content
...
```
<tool>write</tool>
path: {CHECKPOINT_DIR}/{ASIN}/step10.json
content: {"questions":["问题1全文","问题2全文","问题3全文"]}
```
...
4. **凡分数 ≤ 3，必须输出 Intent Enhancement：**
   ...
   - 不堆砌关键词，语言自然
   - **必须指明应修改哪条 bullet（如"建议修改 Bullet 2"）或是否需要新增一条**

⚠️ **评分硬性约束（Critical）：**
- 分数只能是 **0、3、5** 三个值之一，绝对不允许使用 1、2、4 或任何小数
- 如果一条 bullet "几乎达到 5 分但缺少一个细节" → 分数是 **3**，不是 4
- 三档设计是刻意的：强制区分"明确回答"和"需要推断"之间的信号差距
- averageScore 可以是小数（如 3.67），但每题的单独 score 只能是 0/3/5
...
```
<tool>write</tool>
path: {CHECKPOINT_DIR}/{ASIN}/step11.json
content: {"scores":[{"question":"...","score":5,"label":"Directly Addresses","evidence":"..."},{"question":"...","score":3,"label":"Implicitly Addresses","evidence":"...","enhancement":"..."}],"averageScore":4.3}
```
...
**Bullet 优先级顺序（基于 Cosmo 评分动态调整）：**

先检查 F 步骤的 Cosmo 评分结果：得分最低的问题对应的使用场景，应该对应到排序靠前的 bullet。

基础顺序参考（当 Cosmo 评分无明显差异时使用）：
1. 首条：最强差异点或核心技术
...
**动态调整规则：** 若 Cosmo Q1 得 0 分（买家看不到对应内容），则对应该问题的 bullet 内容必须放在第 1 或第 2 条；若 Q2 得 0 分则对应内容放第 2 或第 3 条，以此类推。排序说明需在 explain 字段中注明。

**为每条 bullet 输出：**
- 原文（完整引用）
- 改写版本
- 改写说明（改了什么，为什么，若调整了顺序需注明原因）
...
| ID | 特征 | 缺失判断标准 |
|----|------|------------|
| V9 | Ranking Emphasis | 核心品类词被多个修饰词/无意义词推后（具体判断见下方 V9 判断规则） |
...
✅ **不触发 V9** 的正常结构（Amazon 官方推荐 NPO 结构）：
- `[Brand] [Core Product Type]...` — 品牌后紧跟品类词，如 `LIKIMIO 43"-59" Extendable Round Dining Table`
- `[Brand] [Size] [Core Product Type]...` — 规格前置也可接受

❌ **触发 V9** 的问题结构（核心品类词被推后）：
- `[Brand] Modern Stylish Premium Quality [Core Product Type]` — 3个以上修饰词把品类词推后

**判断方法：** 提取标题中第一个"品类名词"（product type noun）的位置。如果它在品牌词后的第 1-2 个语义节点内 → 不触发 V9。如果被 3 个以上修饰词推后 → 触发 V9。

**p15 Q&A 信号（需说明，无法从 listing 文本直接验证）：**
提示用户检查：产品页 Q&A 区是否有回答 E 步骤 Rufus 问题的内容；无 Q&A 或长期未回复的问题是 Rufus RAG 信号空洞。

**【write 工具调用】** H + I 均完成后立即执行：
```
<tool>write</tool>
path: {CHECKPOINT_DIR}/{ASIN}/step12.json
content: {"violations":[{"id":"V1","severity":"high","rule":"...","matched":"...","explanation":"..."}],"implicit":[{"id":"V10","severity":"medium","rule":"...","matched":"...","explanation":"..."}]}
```
...
```
<tool>write</tool>
path: {CHECKPOINT_DIR}/{ASIN}/step13.json
content: {"issues":[{"factor":"Reviews","current":"248","action":"...","impact":"low"}],"summary":"..."}
```
...
- **运营自行执行**（`execType: "operator"`）：纯文案优化，今天就能改上去
- **需向供应商确认数据**（`execType: "supplier"`）：改写依赖产品实际参数（承重、认证编号、具体尺寸），必须先获取数据再改文案

行动计划末尾单独列出"待确认数据清单"（`pendingData`），汇总所有需要向供应商核实的信息。

**【write 工具调用】** 分析完成后立即执行：
```
<tool>write</tool>
path: {CHECKPOINT_DIR}/{ASIN}/step14.json
content: {"qualityScore":82,"qualityGrade":"B+","plan":[{"priority":"P1","action":"...","location":"...","impact":"...","execType":"operator"},{"priority":"P1","action":"补充承重数据","location":"Bullet 3","impact":"解决 V14","execType":"supplier"}],"pendingData":[{"dataType":"整体承重（lbs）","usedFor":"Bullet 3","purpose":"补充承重声明"}]}
```
...
**所有 step5–step14.json 写入完成，且每步都已收到 `✅ stepN.json 已写入` 确认后**，执行：

```
<tool>exec</tool>
command: node "{SKILL_DIR}\report_gen.js" {ASIN}
```

执行前先用 `read` 工具验证所有 checkpoint 存在：
```
<tool>exec</tool>
command: Get-ChildItem "{CHECKPOINT_DIR}\{ASIN}"
```
应看到 step1.json 至 step14.json 共 14 个文件。若有缺失，重新执行对应分析步骤并写入，再运行 report_gen.js。

---

### Phase 3.5：md 路线（推荐，更简单）

**适用场景：** write 工具无法可靠地写入多个 JSON 文件时，或希望简化流程时。

**流程：**
```
Claude 输出分析结论（按固定 section 标题格式）
  → write 工具一次性写入 analysis.md
  → node md_to_checkpoints.js [ASIN]
    → 自动解析 md → 写入 step5-14.json → 生成 HTML
```

**优势：** 只需一次 write 调用，格式容错性强，不需要 Claude 输出严格 JSON。

#### 第一步：按固定格式输出分析

每个 section 必须以以下标题开头（`##` + 空格 + `STEP_N`，N 后面可跟任意说明文字）：
...
**⚠️ 标题格式是解析器的切割依据，必须严格遵守：**
- 必须用 `##`（两个井号），不能用 `#` 或 `###`
- `STEP_` 和数字之间没有空格，数字后面可以有空格和中文说明
- 10 个 section 必须全部输出，不可跳过

#### 第二步：各 section 内容格式规范

解析器支持 JSON 格式（优先）和自然文本格式（fallback）。**强烈建议在 section 内容里用 JSON，解析最准确。**

**STEP_5 关键词分级** — 推荐 JSON：
````
## STEP_5  关键词分级
```json
{
  "primary": [{"keyword":"bed frame","freq":"28/30 (93%)","note":"核心品类词"}],
  "secondary": [{"keyword":"no box spring","freq":"18/30 (60%)","note":"品类共识"}],
  "backend": ["storage", "heavy duty", "noise free"],
  "sizeSignals": ["14 inch"],
  "competitorCount": 30
}
```
...
{
  "issues": [
    {"severity":"medium","issue":"Bed Frame 重复","detail":"标题里出现两次 'bed frame'，浪费字符"}
  ],
  "spellErrors": [],
  "score": 72,
  "charCount": 185
...
**STEP_7 三版优化标题** — 推荐 JSON：
````
## STEP_7  三版优化标题
```json
{
  "versionA": "Brand Queen Bed Frame...",
  "versionAChars": 162,
  "versionANote": "最大关键词覆盖",
  "versionB": "Brand Queen Metal Bed Frame...",
  "versionBChars": 148,
  "versionBNote": "高 CTR，功能前置",
  "versionC": "Brand Queen Bed Frame – No Box Spring",
  "versionCChars": 72,
  "versionCNote": "移动端优化"
}
```
...
**STEP_8 Backend Keywords** — 推荐 JSON：
````
## STEP_8  Backend Keywords
```json
{"backend":"metal platform bed frame noise free heavy duty storage 14 inch","byteCount":192,"charLimit":250}
```
````

**STEP_9 Bullet 改写** — 推荐 JSON：
````
## STEP_9  Bullet 改写
```json
{
  "bullets": [
    {
      "original": "原文...",
      "rewrite": "改写版本...",
      "explain": "改了什么，为什么（来源：step2 bullet 1）",
      "factCheck": {"passed":true,"claims":[{"claim":"800LBS","verified":true,"source":"title"}]}
    }
  ]
}
```
...
**STEP_10 Rufus 意图问题** — 推荐 JSON：
````
## STEP_10 Rufus 意图问题
```json
{"questions":["问题1全文？","问题2全文？","问题3全文？"]}
```
````
注意：必须 **EXACTLY 3 个**问题。

**STEP_11 Cosmo 内容评分** — 推荐 JSON：
````
## STEP_11 Cosmo 内容评分
```json
{
  "scores": [
    {"question":"问题1全文","score":5,"label":"Directly Addresses","evidence":"Bullet 2: '...'"},
    {"question":"问题2全文","score":3,"label":"Implicitly Addresses","evidence":"...","enhancement":"建议修改 Bullet 3：..."},
    {"question":"问题3全文","score":0,"label":"Missing","evidence":"无任何 bullet 涉及","enhancement":"建议新增一条：..."}
  ],
  "averageScore": 2.67
}
```
````
注意：score 只能是 **0、3、5**，不允许其他值。

**STEP_12 违规检测** — 推荐 JSON：
````
## STEP_12 违规检测
```json
{
  "violations": [],
  "implicit": [
    {"id":"V14","severity":"medium","rule":"Authoritativeness 缺失","matched":"strong and sturdy","explanation":"无具体承重数据，买家无法判断"}
  ]
}
```
````

**STEP_13 Listing Weight** — 推荐 JSON：
````
## STEP_13 Listing Weight
```json
{
  "issues": [
    {"factor":"Reviews","current":"248","action":"加入 Vine 项目 + 售后邮件催评","impact":"medium"}
  ],
  "summary": "Listing Weight 中等，评论数是主要短板。"
}
```
````

**STEP_14 行动计划** — 推荐 JSON：
````
## STEP_14 行动计划
```json
{
  ...
  "qualityGrade": "C+",
  "plan": [
    {"priority":"P1","action":"在 Bullet 3 补充承重数据","location":"Bullet 3","impact":"解决 V14，减少买家疑虑","execType":"supplier"},
    {"priority":"P2","action":"删除标题里重复的 'Bed Frame'","location":"Title","impact":"节省 10 字符，减少权重稀释","execType":"operator"}
  ],
  "pendingData": [
    {"dataType":"整体承重（lbs）","usedFor":"Bullet 3","purpose":"补充承重声明"}
  ]
}
```
````

#### 第三步：write 工具写入 analysis.md

```
<tool>write</tool>
path: {CHECKPOINT_DIR}/{ASIN}/analysis.md
content: [上方所有 section 的完整内容，从 ## STEP_5 开始]
```
...
```
<tool>exec</tool>
command: node "{SKILL_DIR}\md_to_checkpoints.js" {ASIN}
```

脚本会自动：解析 analysis.md → 写入 step5-14.json → 调用 report_gen.js 生成 HTML。

**验证：**
```
<tool>exec</tool>
command: node -e "var h=require('fs').readFileSync('{REPORT_DIR}/{ASIN}/{ASIN}.html','utf8'); var n=(h.match(/待 Claude 分析/g)||[]).length; console.log('占位符数量: '+n+(n===0?' ✅':' ❌ 仍有未填充 section'));"
```

---

### Phase 4：对话分析后注入报告（JSON 路线）

**适用场景：** 已有 JSON 格式分析结果时使用。与 Phase 3.5 的 md 路线功能等价，两者选其一即可。

**新增脚本：** `inject_analysis.js` — 读取 analysis.json，写入所有 step5-14 checkpoint，重新生成 HTML。
...
```json
{
  "step5":  { "primary": ["cookware set"], "secondary": ["ceramic"], "backend": ["rv pots"] },
  "step6":  { "issues": ["RV属性未前置"], "score": 72 },
  "step7":  { "versionA": "...", "versionAChars": 180 },
  "step8":  { "backend": "rv pots pans", "byteCount": 180 },
  "step9":  { "bullets": [{"original": "...", "rewrite": "...", "explanation": "..." }] },
  "step10": { "questions": ["Q1?", "Q2?", "Q3?"] },
  "step11": { "scores": [{"question": "...", "score": 4 }], "avg": 4.0 },
  "step12": { "explicit": [], "implicit": [] },
  "step13": { "summary": "...", "issues": [] },
  "step14": { "actions": [{"priority": "P0", "action": "...", "location": "title", "impact": "..." }] }
}
```
...
1. 对话完成分析 → 输出完整结论
2. 整理 analysis.json
3. `node inject_analysis.js <ASIN> <analysis.json>`
4. 确认 HTML 无占位符

**验证：** `grep "待 Claude 分析" <报告路径>` 应返回空。
...
- 竞品数据：实时抓取 N 条（若未获取 ≥5 条，分析已自动终止）
...
#### step3.json — 核心产品提取 + 规格信号
```json
{
  "coreProduct": "pull-up bar",
  "brand": "SELEWARE",
  "sizeSignals": ["440 lb", "36 inch"],
  "titleBigrams": [
    { "phrase": "pull up", "score": 2, "catBoost": 1 },
    { "phrase": "up bar", "score": 1, "catBoost": 1 },
    { "phrase": "wall mount", "score": 1, "catBoost": 0 }
  ]
}
```
- `coreProduct`: 核心产品类型（category-weighted bigram 提取，v2.4 改进）
- `brand`: 品牌名
- `sizeSignals`: 规格信号数组（容量/尺寸等）
- `titleBigrams`: title 中提取的 2 词短语，含 score 和 catBoost（category 信号加权）
  ...
  "bullets": [
    {
      ...
      "explain": "Added exact dimensions upfront (source: step2 title)",
      "factCheck": {
        "passed": true,
        "claims": [
          { "claim": "60\"L x 38\"W", "verified": true, "source": "step2 title" }
        ]
      }
    }
  ]
...
```
- `bullets[].original`: 原文
- `bullets[].rewrite`: 改写版本（所有事实已经过 G.5 Fact-Check 验证）
- `bullets[].explain`: 改写说明（需注明事实来源）
- `bullets[].factCheck`: Fact-Check 结果对象
  - `passed`: 是否全部声明均已验证
  - `claims[]`: 每条事实声明的验证记录，含 `claim`、`verified`（bool）、`source` 或 `action`（未验证时说明处理方式）
...
1. **分析顺序不可打乱（严格依赖关系）：**
   ```
   A（关键词）→ B（标题审计）→ C（标题版本）→ D（Backend）
              → E（Rufus问题）→ F（Cosmo评分）→ G（Bullet改写，依赖F评分和Enhancement建议）
              → G.5（Fact-Check）→ H+I（违规检测）→ J（Weight）→ K（行动计划）
   ```
   **G 步骤必须在 F 之后执行**，因为 Bullet 的改写内容和排序都依赖 Cosmo 评分结果。
