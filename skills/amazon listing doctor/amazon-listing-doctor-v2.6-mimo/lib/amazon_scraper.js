...
function createPageContext(cc) {
  ...
  const TZ_MAP = {
    ...
    IN: 'Asia/Kolkata', MX: 'America/Mexico_City'
  };
  return {
...
async function scrapeProductPage(asin, cc) {
  ...
  const result = {
    ...
    marketplace: cc, url: url, asin: asin, error: null
  };

  try {
    browser = await createBrowser(cc);
    const page = await browser.newPage(createPageContext(cc));
...
// ── Competitor Item Price/Rating Extractor ────────────────────
/**
 * Extract price and rating from a competitor item element (cheerio scope)
 * @param {Cheerio} $ - cheerio instance scoped to parent element
 * @param {Cheerio} el - the item element
 * @returns {{price: string|null, rating: string|null}}
 */
function extractCompPriceRating($, el) {
  var price = null;
  var rating = null;
  ...
  var priceEl = el.find('.a-price .a-offscreen').first();
  if (!priceEl.length) priceEl = el.find('.a-offscreen[data-a-color="price"]').first();
  if (!priceEl.length) priceEl = el.find('span[data-a-color="price"] .a-offscreen').first();
  if (!priceEl.length) priceEl = el.find('.a-price-whole').first();
  if (!priceEl.length) priceEl = el.find('.a-color-price').first();
  if (priceEl.length) {
    price = priceEl.text().replace(/[^0-9.,]/g, '').trim();
    if (!price) price = null;
  }

  // Rating — the star rating text (e.g. "4.5 out of 5 stars")
  var ratingEl = el.find('.a-icon-alt').first();
  if (!ratingEl.length) ratingEl = el.find('i.a-icon-star .a-icon-alt').first();
  if (!ratingEl.length) ratingEl = el.find('[data-a-star-rating]').first();
  if (ratingEl.length) {
    rating = ratingEl.text().replace(/^\s+|\s+$/g, '').trim();
    if (!rating || !rating.match(/\d/)) rating = null;
  }

  // Shipping price — look for shipping cost in various formats
  var shippingEl = el.find('.a-color-secondary').first();
  if (!shippingEl.length) shippingEl = el.find('.a-size-base.a-color-secondary').first();
  if (!shippingEl.length) shippingEl = el.find('.a-text-bold').first();
  if (shippingEl.length) {
    var shippingText = shippingEl.text();
    // Look for patterns like "$5.99 shipping", "+$X.XX shipping", "Shipping $X.XX"
    var shippingMatch = shippingText.match(/\$([\d,]+\.?\d*)\s*shipping/i) ||
                        shippingText.match(/\+\$([\d,]+\.?\d*)/i) ||
                        shippingText.match(/shipping\s*\$([\d,]+\.?\d*)/i);
    if (shippingMatch) {
      shippingPrice = shippingMatch[1].replace(/,/g, '');
    }
  }

  // Free Shipping detection: Prime badge or "FREE Shipping" text
  if (el.find('.a-icon-prime, .a-icon-fba, [aria-label*="Prime"]').length > 0) {
    freeShipping = true;
  }
  if (!freeShipping) {
    var itemText = el.text() || '';
    if (/free\s*shipping/i.test(itemText)) freeShipping = true;
  }

  // Calculate total price: product price + shipping (if not free)
  // Note: Amazon search results rarely show actual shipping costs (only 'FREE Shipping' or nothing).
  // If shippingPrice is null and not free shipping, we cannot determine the true total price.
  // We set totalPrice to null in this case to avoid misleading data.
  if (price) {
    var priceNum = parseFloat(price.replace(/,/g, ''));
    if (freeShipping) {
      totalPrice = priceNum;
    } else if (shippingPrice) {
      var shippingNum = parseFloat(shippingPrice);
      totalPrice = priceNum + shippingNum;
    } else {
      // Unknown shipping cost: cannot calculate accurate total price
      // Set to null to indicate uncertainty
      totalPrice = null;
    }
  }

  return { price, rating, freeShipping, shippingPrice, totalPrice: totalPrice !== null ? totalPrice.toFixed(2) : null };
}
...
async function scrapeCompetitorSearch(keyword, cc, opts) {
  ...
  try {
    ...
    for (let ri = 0; ri < plan.length; ri++) {
      const round = plan[ri];
      // Skip single-word search rounds entirely — they're too generic (maker, machine, etc.)
      // and reliably trigger Amazon bot detection or return irrelevant results.
      // Multi-word phrases are product-type keywords and always run.
      ...
      try {
        const page = await browser.newPage(createPageContext(cc));
        // Visit homepage first to establish session cookies (avoids 503 on direct search)
        await page.goto('https://www.' + domain, { waitUntil: 'domcontentloaded', timeout: timeout });
        await page.waitForTimeout(3000);
        ...
        try {
          const consentSelectors = [
            '#sp-cc-accept', '#sp-cc-accept-all',
            'button:has-text("Accept")', 'button:has-text("Akzeptieren")',
            'button:has-text("Tout accepter")', 'button:has-text("Accetta")',
            'button:has-text("Aceptar")', '#cookieConsentButton'
          ];
          for (var ci = 0; ci < consentSelectors.length; ci++) {
            try {
              var cBtn = await page.$(consentSelectors[ci]);
              if (cBtn && await cBtn.isVisible()) {
        ...
        $('[data-asin]').each(function() {
          ...
          seenAsin.add(asin);
          var { price, rating, freeShipping, shippingPrice, totalPrice } = extractCompPriceRating($, el);
          allCompetitors.push({ asin, title, price, rating, freeShipping, shippingPrice, totalPrice });
          found++;
        ...
        // Strategy 3: Amazon 2025 New UI — data-cy="title-recipe"
        $('div[data-cy="title-recipe"]').each(function() {
          const el = $(this).closest('[data-asin], [data-csa-c-type="item"]').length
            ? $(this).closest('[data-asin], [data-csa-c-type="item"]')
            : $(this).parent().parent();
          const asin = el.attr('data-asin') || '';
          const asinRaw = el.attr('data-csa-c-item-id') || '';
          const asinMatch = asinRaw.match(/amzn1\.asin\.?(\d+\.)?([A-Z0-9]{10})/i);
          const finalAsin = asin || (asinMatch ? asinMatch[2] : '');
          if (!finalAsin || finalAsin.length < 8) return;
          if (seenAsin.has(finalAsin)) return;

          let title = trimText($(this).text()) || '';
          title = title.replace(/\s*[\d.]+\s+out\s+of\s+\d+\s+stars.*$/i, '').trim();
          title = title.replace(/\s*#\d+\s+in\s+[^|]*\|.*$/i, '').trim();
          title = title.replace(/\s*Sponsored.*$/i, '').trim();
          if (!title || title.length < 5) return;

          seenAsin.add(finalAsin);
          var { price, rating, freeShipping, shippingPrice, totalPrice } = extractCompPriceRating($, el);
          allCompetitors.push({ asin: finalAsin, title, price, rating, freeShipping, shippingPrice, totalPrice });
          found++;
          if (found >= maxPerRound) return false;
          if (allCompetitors.length >= maxCompetitors) return false;
        });
    ...
    // Extra keyword search rounds — use foundPhrases 2-word product terms if cascade returned few results
    if (extraKeywords.length > 0 && allCompetitors.length < 10) {
      console.error('  Extra keyword search rounds (from foundPhrases):');
      let roundNum = plan.length + 1;
      for (let ki = 0; ki < extraKeywords.length; ki++) {
        const kw = extraKeywords[ki];
        if (seenAsin.has(kw)) continue; // skip already-searched
        try {
          const page = await browser.newPage(createPageContext(cc));
          const searchUrl = 'https://www.' + domain + '/s?k=' + encodeURIComponent(kw) + '&s=' + sort;
          await page.goto(searchUrl, { waitUntil: 'domcontentloaded', timeout: timeout });
          await page.waitForTimeout(5000);
          // Cookie consent (universal-scraper v2.0 style)
          try {
            ...
            for (var sci = 0; sci < consentSel.length; sci++) {
              try { var cb = await page.$(consentSel[sci]); if (cb && await cb.isVisible()) { await cb.click({ timeout: 3000 }); await page.waitForTimeout(2000); break; } } catch(e) {}
            }
          } catch(e) {}
          await handleInterstitial(page);
          if (cc === 'US') { await zipAndReload(page, config, timeout); }
          const html = await page.content();
          const $ = cheerio.load(html);
          let found = 0;
          $('[data-asin]').each(function() {
            ...
            const asin = el.attr('data-asin');
            if (!asin || asin === 'SEARCH_SIZE_AD' || asin.length < 8) return;
            const isAd = el.attr('data-ad') || el.attr('data-sponsored') || el.find('[data-ad], [data-sponsored]').length > 0;
            if (isAd) return;
            let title = trimText(el.find('h2 > span').first().text()) || trimText(el.find('h2 span').first().text()) || trimText(el.find('.a-link-normal span').first().text()) || '';
            title = title.replace(/\s*[\d.]+\s+out\s+of\s+\d+\s+stars.*$/i, '').trim().replace(/\s*#\d+\s+in\s+[^|]*\|.*$/i, '').trim().replace(/\s*Sponsored.*$/i, '').trim();
            if (!title || title.length < 5) return;
            if (seenAsin.has(asin)) return;
            seenAsin.add(asin);
            var { price, rating, freeShipping, shippingPrice, totalPrice } = extractCompPriceRating($, el);
          ...
          $('[data-csa-c-type="item"][data-cel-widget*="MAIN-SEARCH_RESULTS"]').each(function() {
            ...
            const titleEl = el.find('h2 > span').first();
            let title = titleEl.length ? trimText(titleEl.text()) : '';
            if (isBrandOnlyTitle(title)) { const linkEl = el.find('a[href*="/dp/"]').first(); const url = linkEl.length ? linkEl.attr('href') || '' : ''; const urlTitle = titleFromUrl(url); if (urlTitle && urlTitle.length > title.length) { title = urlTitle; } }
            if (!title || title.length < 5 || title.match(/^\d/)) return;
            title = title.replace(/\s*[\d.]+\s+out\s+of\s+\d+\s+stars.*$/i, '').trim().replace(/\s*#\d+\s+in\s+[^|]*\|.*$/i, '').trim().replace(/\s*Sponsored.*$/i, '').trim();
            seenAsin.add(asin);
            var { price, rating, freeShipping, shippingPrice, totalPrice } = extractCompPriceRating($, el);
          ...
          cascadeRounds.push({ round: roundNum, keyword: kw, found, source: 'foundPhrases' });
          console.error('  Extra Round ' + roundNum + ': "' + kw + '" found=' + found + ' total=' + allCompetitors.length);
          await page.close();
          roundNum++;
          if (allCompetitors.length >= maxCompetitors) break;
        } catch(e) {
          console.error('  Extra Round error for "' + kw + '":', e.message.split('\n')[0]);
        }
      }
    }
  } finally {
