...
async function refineOverview(draft, bullets) {
  return new Promise(function(resolve) {
    ...
    var body = JSON.stringify({
      model: config.llm.models.default, max_tokens: 512,
      system: 'You are a Wayfair listing writer. Rewrite the draft in professional third-person style. No marketing language. No "you will love". Be objective and factual. Output only the rewritten text.',
      messages: [{ role: 'user', content: 'Draft:\n' + draft + '\n\nAdditional context from bullets:\n' + bulletsText + '\n\nRewrite in Wayfair style:' }]
    });
    var req = http.request({
      ...
      res.on('end', function() {
        try {
          ...
          resolve(text.length > 20 ? text : draft);
        } catch(e) { resolve(draft); }
      });
    });
    req.setTimeout(45000, function() { req.destroy(); resolve(draft); });
    req.on('error', function() { resolve(draft); });
