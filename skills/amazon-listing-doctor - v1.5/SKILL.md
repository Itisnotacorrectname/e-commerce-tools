...
version: 6.0.0
description: Diagnose any Amazon listing with a full competitive audit, Rufus intent simulation, Cosmo content scoring, and rewritten bullet copy. Paste any amazon.com product link to run. Fully automated (data + analysis + report) — no manual steps required.
---
...
本 skill 采用**三层自动化**架构（v1.5+）：

```
数据层（diagnose.js）    → step1-4 checkpoint
分析层（analyze.js）     → step5-14 checkpoint  ← NEW: 规则引擎，自动运行
渲染层（report_gen.js）  → HTML 报告
```
...
### ⚠️ Checkpoint 写入规范（每步必读）

**每完成一个分析步骤，必须立即将结果写入对应的 checkpoint 文件，不可等到全部分析完成后再批量写入。**

**路径规则：**
```
CHECKPOINT_DIR = ~/.openclaw/workspace/amazon-listing-doctor/checkpoints/[ASIN]/
```
...
**写入指令模板（每步末尾必须执行）：**
```
将以下 JSON 写入文件：~/.openclaw/workspace/amazon-listing-doctor/checkpoints/[ASIN]/step[N].json
（若目录不存在，先创建目录）
内容：[本步骤输出的完整 JSON 对象]
```
...
- 写入前检查：所有必填字段是否存在，字段类型是否匹配
- 写入成功后在对话中确认：`✅ step[N].json 已写入`，失败则报错并重试
...
**【checkpoint 写入】** 完成本步骤后，将结果按 `step5.json` schema 写入：
`~/.openclaw/workspace/amazon-listing-doctor/checkpoints/[ASIN]/step5.json`
...
**【checkpoint 写入】** 完成本步骤后，将结果按 `step6.json` schema 写入：
`~/.openclaw/workspace/amazon-listing-doctor/checkpoints/[ASIN]/step6.json`
...
**【checkpoint 写入】** 完成本步骤后，将结果按 `step7.json` schema 写入：
`~/.openclaw/workspace/amazon-listing-doctor/checkpoints/[ASIN]/step7.json`
...
**【checkpoint 写入】** 完成本步骤后，将结果按 `step8.json` schema 写入：
`~/.openclaw/workspace/amazon-listing-doctor/checkpoints/[ASIN]/step8.json`
...
**【checkpoint 写入】** G + G.5 均完成后，将结果按 `step9.json` schema 写入：
`~/.openclaw/workspace/amazon-listing-doctor/checkpoints/[ASIN]/step9.json`
（包含 `factCheck` 字段，所有 bullet 的验证记录完整）
...
**【checkpoint 写入】** 完成本步骤后，将结果按 `step10.json` schema 写入：
`~/.openclaw/workspace/amazon-listing-doctor/checkpoints/[ASIN]/step10.json`
...
**【checkpoint 写入】** 完成本步骤后，将结果按 `step11.json` schema 写入：
`~/.openclaw/workspace/amazon-listing-doctor/checkpoints/[ASIN]/step11.json`
...
**【checkpoint 写入】** H + I 均完成后，将显性 + 隐性违规结果合并，按 `step12.json` schema 写入：
`~/.openclaw/workspace/amazon-listing-doctor/checkpoints/[ASIN]/step12.json`
...
**【checkpoint 写入】** 完成本步骤后，将结果按 `step13.json` schema 写入：
`~/.openclaw/workspace/amazon-listing-doctor/checkpoints/[ASIN]/step13.json`
...
**【checkpoint 写入】** 完成本步骤后，将结果按 `step14.json` schema 写入：
`~/.openclaw/workspace/amazon-listing-doctor/checkpoints/[ASIN]/step14.json`

---

### Phase 3：生成 HTML 报告

**所有 step5–step14.json 写入完成后**，执行：

```bash
node ~/.openclaw/workspace/skills/amazon-listing-doctor/report_gen.js [ASIN]
```

执行前先确认所有 checkpoint 均已写入：
```bash
ls ~/.openclaw/workspace/amazon-listing-doctor/checkpoints/[ASIN]/
# 应看到 step1.json ~ step14.json 共 14 个文件
```

若有缺失的 step 文件，重新执行对应分析步骤并写入，再运行 report_gen.js。
...
- `byteCount`: 字节数（字段名必须是 `byteCount`，不是 `charCount`）
