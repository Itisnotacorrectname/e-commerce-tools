...
function normalizeStep(stepKey, data) {
  ...
  switch (stepKey) {
    ...
    case 'step14':
      ...
      // pendingData 新字段，不存在时补空数组（向后兼容旧格式）
...
// 注意：验证的是 normalizeStep 之后的字段名（即 report_gen.js 期望的字段名）
const STEP_DEFS = {
  step5:  ['primary','secondary','backend'],
  step6:  ['issues'],                    // spellErrors 由 normalizeStep 补充
  step7:  ['versionA','versionB','versionC'],
  ...
  step13: ['summary','issues'],
  step14: ['plan']                       // 规范化后用 plan；pendingData 可选
};
