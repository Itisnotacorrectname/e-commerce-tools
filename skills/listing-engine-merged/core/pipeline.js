...
async function run(input, options) {
  ...
  context = await runLayer(context, 'layer2', 'extract_features', async function(ctx_) {
    return l2.extractProductFeatures(ctx_);
  }, { force: force });
...
function generate(urlOrAsin, targetPlatforms, options) {
  var input = typeof urlOrAsin === 'string' && urlOrAsin.startsWith('http')
    ? { url: urlOrAsin }
    : { asin: urlOrAsin };
