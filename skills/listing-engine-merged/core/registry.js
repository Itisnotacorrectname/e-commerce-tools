...
function registerDefaults(baseDir) {
  ...
  // Resolve each module path relative to the skill root (parent of core/)
  Object.keys(DEFAULT_MODULES).forEach(function(key) {
    // DEFAULT_MODULES paths are relative to skill root, so resolve from baseDir
    var resolvedPath = path.resolve(baseDir, DEFAULT_MODULES[key]);
    register(key, resolvedPath);
  });
}
