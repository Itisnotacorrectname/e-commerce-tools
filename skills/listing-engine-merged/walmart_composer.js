...
function cleanFeature(text) {
  if (!text) return '';
  // Remove Amazon format: 【STURDY】 or [STURDY] at start, and inline markers
  return text
    .replace(/^【[^】]+】\s*/g, '')
    .replace(/^\[[^\]]+\]\s*/g, '')
    .replace(/【[^】]+】/g, ' — ')
...
function buildKeyFeatures(context, constraints) {
  ...
  var attrs    = (context.product && context.product.attributes) || {};

  var items = [];

  // 优先使用 conversion 层生成的消息（handle both string and {message:...} object）
  messages.forEach(function(m) {
    var text = (typeof m === 'string') ? m : (m.message || m.text || '');
    if (!text) return;
    var cleaned = cleanFeature(text);
    if (cleaned.length <= constraints.keyFeatures.maxCharsPerItem) {
      items.push(cleaned);
    } else {
      items.push(cleaned.substring(0, constraints.keyFeatures.maxCharsPerItem - 3) + '...');
    }
  });

  // 补充原始 bullets（清洗后），按 Walmart 格式重新组织
  if (items.length < constraints.keyFeatures.min) {
    bullets.forEach(function(b) {
      if (!b || items.length >= constraints.keyFeatures.max) return;
      var cleaned = cleanFeature(b);
      // 取第一句
      var firstSentence = cleaned.split(/[.!?]/)[0].trim();
      ...
      if (firstSentence.length > 15) items.push(firstSentence);
    });
  }

  // 加入 specs 作为额外 features（weight_capacity, material 等）
  if (items.length < constraints.keyFeatures.max) {
    var mats = attrs.materials && attrs.materials.raw;
    if (mats && mats.length > 0 && items.length < constraints.keyFeatures.max) {
      items.push('Made with ' + mats[0] + ' for lasting durability');
    }
    var colors = attrs.colors && attrs.colors.raw;
    if (colors && colors.length > 0 && items.length < constraints.keyFeatures.max) {
      items.push('Available in ' + colors.join(', ') + ' finishes');
    }
  }
